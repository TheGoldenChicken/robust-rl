...
class CliffCar:
    ...
    ACTIONS = [np.array([0,0]),np.array([1,0]),np.array([0,1]),np.array([-1,0]),np.array([0,-1])]
    CLIFF_PENALTY = -1
    ACTION_DIM = len(ACTIONS)
    OBS_DIM = 2
    START_POSITION = np.array([1,1])
    GOAL_POSITION = np.array([14,1])
    BOUNDS = np.array([-5,-5,20,15])
    CLIFF_HEIGHT = 0
    SPEED = 0.5
    ...
    def result(self, position, action):
        ...
        position = position + self.SPEED * action + noise
...
class CliffCarDiscrete(CliffCar):
    ...
    # These needs to be defined here since the values in CliffCar may be continous
    START_POSITION = np.array([1,1])
    GOAL_POSITION = np.array([14,1])
    BOUNDS = np.array([-3,-2,18,12])
    CLIFF_HEIGHT = 0

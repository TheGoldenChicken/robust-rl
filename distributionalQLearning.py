...
def pre_sub_robust_estimator(X_p,y_p,X_v,y_v,delta,gamma):
    ### Quadratic approximation ###
    A, b, c = quadratic_approximation(X_v, y_v)

    ### Gaussian approximation ###
    mu = np.mean(y_v, axis = 0)
    Sigma = np.cov(y_v.T)

    ### Expectation ###
    exp = lambda beta : expectation(A, b, c, beta, mu, Sigma)
    
    ### Pre supremum term ###
    pre_sup = lambda beta : -beta*np.log(expectation(beta))-beta*delta

    return pre_sup

def pre_sub_robust_estimator_prime(X_p,y_p,X_v,y_v,delta,gamma, tol = 1e-3):
    return lambda beta : (pre_sub_robust_estimator(X_p,y_p,X_v,y_v,delta,gamma)(beta+tol)- \
...
def robust_estimator(X_p, y_p, X_v, y_v, delta, gamma):
    ...
    X_v: Samples (s,a) corresponding to state values in y_v
    y_v: State values V(s) =max_a Q(s,a)
    delta: Kullback liebler divergence distance
    gamma: Gamma discount factor
    """

    f = pre_sub_robust_estimator(X_p,y_p,X_v,y_v,delta,gamma)
    f_prime = pre_sub_robust_estimator_prime(X_p,y_p,X_v,y_v,delta,gamma)

    return f(maximize(f_prime))

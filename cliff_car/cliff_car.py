...
class Env(rl.env.ContinuousEnv):
    ...
    def reset(self):
        for player in self.players:
            for i in range(len(self.layout)):
                for j in range(len(self.layout[i])):
                    if(self.layout[i][j] == 'S'): player.reset((j,i))
    ...
    def render(self):
        ...
        for player in self.players:
            self.display.drawImage('cliff_car\Car.png', (player.position[0]*gs + gs/2, player.position[1]*gs + gs/2), (100,100))

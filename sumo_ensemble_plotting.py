...
def plot_average_q_of_seeds(seeds, delta_vals, linear=True, axis = None, DQN = False):
    ...
    if not DQN:
        ...
        paths = [[f'sumo/test_results/DQN_sumo-12000-frames/seed-{seed}-q_vals.npy' for seed in seeds]]
...
def plot_q_and_state_hist(test_games = 200, DQN = False):
    for delta in deltas:
        ...
        if not DQN:
            ...
            paths_linear = [f'sumo/test_results/DQN_sumo-12000-frames/seed-{seed}-model'
                            for seed in seeds]

            q_paths = [f'sumo/test_results/DQN_sumo-12000-frames/seed-{seed}-q_vals.npy'
                        for seed in seeds]
        ...
        if not DQN:
            ...
            # Increase surrounding white space of plot
            plt.tight_layout()
            plt.savefig(f'plots/q-vals/DQN-ensemble-q-and-state.png', dpi=300)
            break
...
def plot_DQN_performance(test_games = 200):
    for seed in seeds:
        ...
        model_path = f'sumo/test_results/DQN_sumo-12000-frames/seed-{seed}-model'

        q_path = f'sumo/test_results/DQN_sumo-12000-frames/seed-{seed}-q_vals.npy'
